...
(function () {
  ...
  var player = {
    ...
    weapon: 'R', shootTimer: 0, invuln: 0, ducking: false,
    walkPhase: 0
  };
  ...
  function update() {
    ...
    if (jumpK && player.onGround) { player.vy = JUMP_V; player.onGround = false; if (window.JungleStrikeSound) window.JungleStrikeSound.jump(); }
    ...
    if (!boss && stageDist < st.length - W) {
      ...
      if (spawnTimer <= 0) {
        ...
        var sx = camX + W + 20;
        enemies.push(spawnEnemy(type, sx));
      }
